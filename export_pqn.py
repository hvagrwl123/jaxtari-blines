...
api = wandb.Api()
os.makedirs("pqn_data", exist_ok=True)

for run in api.runs("raef-tu-darmstadt/jaxatari-pqn-final"):
    if run.state != "finished":
        print("skipping", run.name, run.state)
        continue
    h = run.history(samples=2000)
    h.to_csv(f"pqn_data/{run.name}.csv", index=False)
...
for env in ["Pong-v5", "BeamRider-v5", "MsPacman-v5"]:
    ...
    for r in runs:
        ...
        h.to_csv(f"pqn_data/cleanrl_{env}_seed{r.config.get('seed')}.csv", index=False)
